...
class _MyHomePageState extends State<MyHomePage> {
    ...
    return FutureBuilder<cti.Album>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          if (snapshot.data!.message == "No matching path found") {
    ...
    return FutureBuilder<cti.ShapeAlbum>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          if (snapshot.data!.stops.isNotEmpty) {
            mapMarkers = [];
  ...
  */
    ...
    if (polylines.isEmpty || (!drawRedLine && polylines.length == 1)) {
      if (polylines.isNotEmpty) {
        predictedLocation = LatLng(
            polylines[0].points.last.latitude,
            polylines[0].points.last.longitude
        );
      }
      return SfMaps(
        layers: [
  ...
  Widget _getAppBarTitle() {
    ...
    if (_title.length * 10 > MediaQuery.of(context).size.width - 64) {
      return SizedBox(
